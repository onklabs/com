...
const TIMEOUTS = {
  WAITING: 30000,        // 30s in queue (original)
  MATCH: 300000,         // 5 minutes match lifetime (original)
  HEARTBEAT: 60000,      // 1 minute heartbeat (original)
  SIGNAL: 30000,         // 30s for signals (original)
  CLEANUP_INTERVAL: 60000 // Clean up every minute (original)
};
...
function createLightweightMatch(peer1, peer2, now) {
  return {
    ...
    st: 'signaling',
    to: {
      o: now + 20000,  // 20s for offer (original)
      a: now + 20000,  // 20s for answer (original)
      c: now + 90000   // 90s for connection (original)
    },
    s: {
      [peer1]: { o: [], a: [], i: [], k: [] },
      [peer2]: { o: [], a: [], i: [], k: [] }
...
async function handleFindMatch(data, now) {
  ...
  if (existingMatchId) {
    ...
    if (existingMatch) {
      ...
      // Reset match status to allow reconnection (back to original)
